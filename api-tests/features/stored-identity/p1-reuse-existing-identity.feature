...
Feature: Stored Identity - Update Existing Identity
  Rule: User does not have an existing SI record
    Background:
    ...
    Rule: Update journey - no existing SI record
  ...
  Rule: User has existing SI record
    Background: New P1 journey to get SI record
    ...
    Scenario: Reuse P1 journey with update
      When I start a new 'low-confidence' journey
      Then I get a 'page-ipv-reuse' page response
