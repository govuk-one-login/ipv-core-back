...
export const enqueueVc = async (
    ...
    const request = await generateDcmawAsyncVcCreationBodyFromScenario(userId, CRI_ID, scenario);
...
export const enqueueError = async (
    ...
    const response = await fetch(
        `${CRI_URL}/management/enqueueError`,
        {
            method: "POST",
            body: JSON.stringify(body),
            redirect: "manual",
    ...
    if (response.status !== 201) {
        throw new Error(`DCMAW enqueue error request failed: ${await response.text()}`);
    }
...
export const cleanUpDcmawState = async (userId: string): Promise<void> => {
    const response = await fetch(
        `${CRI_URL}/management/cleanupDcmawState`,
        {
            method: "POST",
            body: JSON.stringify({
                user_id: userId,
