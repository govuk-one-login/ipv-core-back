...
EVCS_STUB_API_KEY=$(aws secretsmanager get-secret-value --secret-id /"$ENVIRONMENT"/core/evcs/apiKey | jq -r .SecretString)
...
MANAGEMENT_TICF_API_KEY=$(aws secretsmanager get-secret-value --secret-id /"$ENVIRONMENT"/core/credentialIssuers/ticf/connections/stub/apiKey | jq -r .SecretString)
export MANAGEMENT_TICF_API_KEY

MANAGEMENT_CIMIT_STUB_API_KEY=$(aws ssm get-parameter --name /tests/core-back-"$ENVIRONMENT"/cimit_api_key | jq -r .Parameter.Value)
export MANAGEMENT_CIMIT_STUB_API_KEY

CIMIT_INTERNAL_API_KEY=$(aws secretsmanager get-secret-value --secret-id /"$ENVIRONMENT"/core/cimitApi/apiKey | jq -r .SecretString)
